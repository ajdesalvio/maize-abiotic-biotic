...
# Import VI FPCs
fpc <- fread(paste0(data_path, '2021_Rust_FPC_Scores_VI_BLUEs_MissingData.csv')) %>% as.data.frame()

# Work with the median values
fpc <- fpc %>% filter(str_detect(Vegetation.Index, 'median'))

# Unique experiments
exps <- sort(unique(fpc$Experiment))
...
# For each VI/ending flight combination, keep only the FPCs needed to explain 95% of FVE
vi.names <- sort(unique(fpc$Vegetation.Index))
fpc.list <- list()
for (vi in vi.names) {
  for (end in end.flight) {
    # Filter for the current VI/ending flight combination
    fpc_i <- fpc %>% filter(Vegetation.Index == vi, Ending.Flight == end)
    # Pull out the number of FPCs to retain
    num_fpcs_retain <- fpc_i %>%
      select(matches("^FPC\\d+_FVE$")) %>%
      slice(1) %>%
      pivot_longer(everything(), names_to = "comp", values_to = "fve") %>%
      mutate(idx = parse_number(comp)) %>%   # extract FPC number
      arrange(idx) %>%
      mutate(cum = cumsum(replace_na(as.numeric(fve), 0))) %>%
      summarize(k = { w <- which(cum >= 0.95); if (length(w)) w[1] else dplyr::n() }) %>%
      pull(k)
    # Use that number to filter the fpc data for this iteration
    fpc_i_retain <- fpc_i %>% select(Pedigree.Experiment:Ending.Flight, all_of(paste0('FPC', 1:num_fpcs_retain)))
    # Rename the FPC columns so that they identify the vegetation index
    vi_fpc_colnames <- paste(paste0('FPC', 1:num_fpcs_retain), vi, sep = '.')
    colnames(fpc_i_retain)[7:ncol(fpc_i_retain)] <- vi_fpc_colnames
    # Remove the columns that will mess up the inner join later
    fpc_i_retain <- fpc_i_retain %>% select(!Experiment.VI:Vegetation.Index)
    # Save the data frames in wide format
    fpc.list[[paste0(vi, '.Ending.on.', end)]] <- fpc_i_retain
  }
}

# We need to create separate data frames for each FPCA ending type
fpc.list.grouped <- list()
for (end in end.flight) {
  fpc.list.grouped_i <- list()
  fpc.list.grouped_i[[end]] <- fpc.list[grep(end, names(fpc.list))]
  fpc.list.grouped[[end]] <- fpc.list.grouped_i
}

# Create wide format data frames for each FPCA ending type 'keys' defines the common keys for the inner join
keys <- c('Pedigree.Experiment', 'Pedigree', 'Experiment', 'Ending.Flight')
fpc.wide.lists <- list()
for (end in end.flight) {
  fpc.wide.lists[[end]] <- reduce(fpc.list.grouped[[end]][[end]], ~ inner_join(.x, .y, by = keys))
}
...
for (heldout.exp in exps) {
  ...
  for (rust.score.num in rust.scores) {
    ...
    for (end in end.flight) {
      ...
      # Join the rust scores with the FPC predictors
      df0 <- rust.filt %>% inner_join(fpc.wide.lists[[end]], by = 'Pedigree.Experiment')
      ...
      vip$FPC <- lapply(strsplit(as.character(rownames(vip)), '\\.'), '[', 1) %>% unlist()
      vip$Vegetation.Index <- lapply(strsplit(as.character(rownames(vip)), '\\.'), '[', 2) %>% unlist()
...
#write_rds(elastic_all_results, file = paste0(data_path, 'RData/20250814_Elastic_Net_FPCs_MissingData.rds'))
...
#fwrite(vip_df, paste0(data_path, '2021_Rust_Elastic_Net_FPCs_VarImp_MissingData.csv'))
#fwrite(loeo_metrics_df, paste0(data_path, '2021_Rust_Elastic_Net_FPCs_Metrics_MissingData.csv'))
